...
Ramps renders its card per-request from `/api/og`, because a palette _is_
...
the tool produces. Drawing it in the app is what keeps the waveforms _real_
...
g.fillText("UI Sound", 72, 258)
g.fillText("Generator", 72, 336)
...
;[
  ...
  "license.",
].forEach((l, i) => g.fillText(l, 72, 404 + i * 38))
...
for (const [i, id] of picks.entries()) {
  ...
  let pk = 0
  for (let n = 0; n < raw.length; n++) pk = Math.max(pk, Math.abs(raw[n]))
  ...
  const x0 = 600 + OFFSETS[i],
    y0 = top + i * (rowH + gap),
    w = W - x0 + 40
  g.save()
  g.beginPath()
  g.roundRect(x0, y0, w, rowH, 14)
  g.clip()
  g.fillStyle = "#eef3ff"
  g.fillRect(x0, y0, w, rowH)
  g.fillStyle = ASH
